...
class Gallery_Admin extends MY_Controller {
    public function __construct(){
        ...
        $this->load->model(array('gallery_model','event_model')); 
    }

    function index(){
        $this->load->model(array('category_model','jenis_kayu_model','photo_category_model'));    
        $this->data['title'] = 'Master Data - Gallery';
        $this->data['content'] = 'gallery_admin/index';
//        $this->data['cagetoryData'] = $this->prepareList($this->category_model->getAll(), 'id', array('ctgr_name'),true);
//        $this->data['jenisKayuData'] = $this->prepareList($this->jenis_kayu_model->getAll(), 'folder_name', array('jenis_kayu_name'),true);
//        $this->data['cagetoryData'] = $this->prepareList($this->photo_category_model->getAll(), 'id', array('ctgr_name'),true);
        $this->data['eventList'] = $this->prepareList($this->event_model->getByCategory(), 'id', array('event_name'),true);
        $eventId ='';
        
//	 if ($this->input->post('role') == 'product') {
//            $eventId = $this->input->post('event_id');
//	 } else if ($this->input->post('role') == 'jenis_kayu') {
//            $eventId = $this->input->post('jns_kayu_name');
//        } else if ($this->input->post('role') == 'project'){
//            $eventId = $this->input->post('prjc_name');
//        }
        
        $eventId = $this->input->post('event_id');
//        if ($eventId != '') {
//            $this->data['showdelete'] = true;
//        }
//        $this->data['type'] = $this->input->post('gallery_type');
//        $this->data['role'] = $this->input->post('role');
    ...
    function saveImages ($dataImage, $eventId) { //remove ($dataImage, $type, $eventId)
        $saveData = array();
        foreach($dataImage['files'] as $index => $value) {
            ...
            $checkData = $this->gallery_model->getByCategory(array('image_name'=>$value['name']));
...
//                $saveData['type'] = $type;
                ...
                $saveData['created_by'] = '';
                $this->gallery_model->add($saveData);
            }
        }
    }
    ...
    function upload() {
        ...
        if ($this->uri->segment(4)) {
            $upload_handler = new UploadHandler(null,true,$this->uri->segment(4));
            $this->saveImages($upload_handler->get(false),$this->uri->segment(4));
        } 
...
//        } 
    ...
    function getDesc() {
        ...
        $desc = '';
        $descData = $this->gallery_model->getByCategory(array('image_name'=>$imgName));
    ...
    function saveDesc() {
        ...
        $this->gallery_model->update($postData);
    ...
    function deleteImage($eventId = '') {
        ...
        $this->load->model(array('gallery_model'));
        if ($eventId != '') {
            $this->gallery_model->delete($this->gallery_model->_table,array('event_id'=>$eventId));
                //unlink the image
            ...
            $this->gallery_model->delete($this->gallery_model->_table,array('image_name'=>$fName));
                //unlink the image
            $realpath = str_replace("\\", "/", realpath('images'));
            unlink($realpath.'/'.$eventId.'/'.$fName);
            unlink($realpath.'/thumbnail/'.$eventId.'/'.$fName);
    ...
    function publishImage() {
        ...
        $this->load->model(array('gallery_model'));
            ...
            $this->gallery_model->update($where);
    ...
    function moveimage() {
        ...
        $this->gallery_model->move_image($where,$postData);
...
class UploadHandler
{
    ...
    protected $category;
    function __construct($options = null, $initialize = true, $eventId) {
        $this->options = array(
            'script_url' => $this->get_full_url().'/gallery/upload/',
...
//        if ($gllrType) {
//            $this->setGalleryType($gllrType);
//        }
        if ($eventId) {
            $this->setCategory($eventId);
        }
        if ($initialize) {
            $this->initialize();
        }
