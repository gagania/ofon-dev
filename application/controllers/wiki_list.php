...
class Wiki_List extends MY_Controller {
    
    public function __construct(){
        parent::__construct();
        date_default_timezone_set('UTC'); 
        $this->load->helper('url');
        $this->load->model(array('wiki_model'));
    }
    
    function index(){
        $this->data['title'] = 'Wiki List';
        $this->data['content'] = 'wiki_list/index';
        $this->data['wikiData'] = $this->wiki_model->getData($this->_limit);
        $this->data['totaldata'] = sizeof($this->wiki_model->getAll());
        $this->data['pnumber'] = 1;
...
//                $pnumber = 1;
        ...
        $totalData = $this->wiki_model->getData(0,$limit,$whereSearch);
        $result = $this->wiki_model->getData($this->_limit,$limit,$whereSearch);
    ...
    function add() {
        ...
        if ($this->uri->segment(3)) { //edited
            ...
            $dataTables = $this->wiki_model->getByCategory($where);
        ...
        $this->data['content'] = 'wiki_list/add';
     ...
     public function save() {
        ...
        if ($new) { // add
            $id = $this->wiki_model->add($postData);
            if (!$id) {
                $success = false;
            ...
            $this->wiki_model->update($postData);
        }
    ...
    function delete() {
        ...
        if (sizeof($data) > 0) {
            foreach($data as $value) {
                $this->wiki_model->delete($this->wiki_model->_table,array('id'=>$value['id']));
            }
