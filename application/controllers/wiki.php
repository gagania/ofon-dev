...
//        date_default_timezone_set('UTC'); 
        ...
        $this->load->model(array('wiki_model','contact_model','wiki_content_model','wiki_menu_model','wiki_reference_model'));
    }
...
//        $this->data['menus'] = $this->getAllMenuWeb('header','frontend'); 
        ...
        $wikiList = $this->wiki_model->getAll();
        $wikiMenus = $this->wiki_menu_model->getLeftMenu();
        $wikiContent = $this->wiki_content_model->getAll();
        $wikiReference = $this->wiki_reference_model->getAll();
        $child = array();
        foreach ($wikiMenus as $index => $value) {
            $child = $this->getChild($value['id']);
            if (sizeof($child) > 0) {
                $wikiMenus[$index]['child'] = $child;
            }
        }

        $this->data['wikiMenus'] = $wikiMenus;
        $this->data['wikilist'] = $wikiList;
        $this->data['wikiContent'] = $wikiContent;
        $this->data['wikiReference'] = $wikiReference;
...
//        $this->data['recentArticle'] = $recentArticle;
        ...
        if($segment_2 == '' && !strpos($segment_1,'.html')){ 
            ...
            $this->data['menu_active'] = 'Wiki'; 
            $this->data['content'] = 'wiki/list';
...
//            $this->data['articleData'] = $articleData;
        ...
        $this->load->view('layout_frontend_wiki', $this->data);
    }
    
    function getChild($parentId) {
        $wikiMenus = $this->wiki_menu_model->getLeftMenu($parentId);
        if (sizeof($wikiMenus) > 0) {
            foreach ($wikiMenus as $index => $value) {
                $child = $this->getChild($value['id']);
                if (sizeof($child) > 0) {
                    $wikiMenus[$index]['child'] = $child;
