        ...
        <div class="portlet box green">
            ...
            <div class="portlet-body">
                <div class="clearfix">
                    ...
                    <div class="btn-group"> 
                        <a class="btn green" data-toggle="modal" href="#upload_user_admin">Upload News Data  <i class="icon-upload"></i></a>
                    </div>
                </div>
                <div class="row-fluid">
                    <div class="">
                        <div class="dataTables_filter">
                            <label>Search: <input type="text" id="search_desc" name="search_desc" onkeyup="javascript:searchdata('news_list','<?=base_url()?>','<?php echo $this->router->fetch_class();?>',this);" class="m-wrap small"></label>
                        </div>
                    </div>
                </div>
                <table class="table table-striped table-bordered table-hover table-full-width" id="news_list">
                    <thead>
                        <tr>
                        ...
                        if ($userData) {
                            foreach ($userData as $index => $value) {
                                ...
                                <tr class="odd gradeX">
                                    ...
                                    <td><a href="#" onclick="javascript:add_data('<?= base_url() ?>','<?php echo $this->router->fetch_class();?>', '<?= $value['id'] ?>');"><?php echo $value['news_title']; ?></a></td>
                                </tr>
                                <?php
                            }
                        }
                ...
                <div style="width:100%">
                    ...
                    <table class="footer-table">
                        <tbody>
                            <tr>
                                <td><button onclick="updatelist('news_list', '<?php echo base_url();?>','<?php echo $this->router->fetch_class();?>','first');" class="btn-first" type="button">&nbsp;</button></td>
                                <td><button onclick="updatelist('news_list','<?php echo base_url();?>','<?php echo $this->router->fetch_class();?>','prev');" class="btn-prev" type="button">&nbsp;</button></td>
                                <td><span class="ytb-sep"></span></td>
                                <td><span class="ytb-text">Page</span></td>
                                <td><input type="text" onkeyup="updatelist('news_list','<?php echo base_url();?>','<?php echo $this->router->fetch_class();?>','page', this.value);" size="3" value="<?php echo ($pnumber) ?$pnumber :1;?>" class="pnumber"></td>
                                <td><span class="ytb-text" id="totaldata_view">of <?php echo round($totaldata/10)?></span></td>
                                <td><span class="ytb-sep"></span></td>
                                <td><button onclick="updatelist('news_list','<?php echo base_url();?>','<?php echo $this->router->fetch_class();?>','next');" class="btn-next" type="button">&nbsp;</button></td>
                                <td><button onclick="updatelist('news_list','<?php echo base_url();?>','<?php echo $this->router->fetch_class();?>','last');" class="btn-last" type="button">&nbsp;</button></td>
                                <td>
